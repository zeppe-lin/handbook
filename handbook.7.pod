...
======================================================================
= AFTER EDITING THIS HANDBOOK, DO NOT FORGET TO UPDATE THE HANDBOOK  =
= PACKAGE IN PKGSRC TREE.  ALSO, UPDATE THE ZEPPE-LIN WEBSITE, SINCE =
= IT IS GENERATED FROM THIS HANDBOOK:                                =
=                                                                    =
...
<img alt="Zeppe-Lin Logo" class="logo">
...
It's also available on the Zeppe-Lin system as
...
=head3 Download rootfs tarball
  ...
  # cd /mnt
  # wget -c https://github.com/zeppe-lin/pkgsrc/releases/download/2023-01-02/rootfs-2023-01-02-x86_64.tar.xz{,.sig}

=head4 Signature verification

  # gpg --keyserver keyserver.ubuntu.com --recv-keys 59ec1986fbd902cf
  # gpg --verify rootfs-2023-01-02-x86_64.tar.xz.sig rootfs-2023-01-02-x86_64.tar.xz

=head3 Extract rootfs tarball

  # tar -xvf rootfs-2023-01-02-x86_64.tar.xz -C .
...
five official pkgsrc repositories (collections):
...
required to create a basic Zeppe-Lin system.  Most likely you need it.

=item C<system>

This collection contains build scripts and files for the packages
required for minimal Zeppe-Lin installation on real hardware: boot
loader, filesystem utilities, hw-monitors, servers, and so on.
C<system> depends on the C<core> collection.

=item C<xorg>

This collection contains build scripts and files for the packages
required for X Window System applications.  C<xorg> depends on the
C<system> collection.
...
manager, chat messenger, etc.  C<desktop> depends on the C<xorg>
collection.

=item C<stuff>

This collection contains build scripts and files for the packages that
B<do not> fit into any of the collections above.

=back

Clone the collections you need, but note that one collection depends
on another.  Example:
...
default only C<core> collection is enabled.  See L<pkgman.conf(5)> for
more information.
...
the I<tar.bz2> extension or B<XZ> ending with I<tar.xz>.
...
We can tell L<pkgman(1)> not to skip the C<fails>, but to stop.  Then
...
installing next in the listed order.  At the end of installation
procedure, all skipped packages will be in the report.

=head4 Viewing and updating outdated packages

Since the packages sources for Zeppe-Lin is distributed via L<git(1)>,
the first thing to do is to update the C<pkgsrc> collection:

  $ cd /usr/src/pkgsrc-core     &&  git pull
  $ cd /usr/src/pkgsrc-system   &&  git pull
  $ cd /usr/src/pkgsrc-xorg     &&  git pull
  $ cd /usr/src/pkgsrc-desktop  &&  git pull

It's may be annoying to do this few steps every time you want to
...
operation.  By default, the I<core> collection is enabled, and
...
file called Pkgfile (see L<Pkgfile(5)> for more info), which contains
information about the package (such as name, version, etc) and the
commands that should be executed in order to compile the package in
question.  To be more specific, the Pkgfile file is actually a POSIX
L<sh(1p)> script, which defines a number of variables (name, version,
release and source) and a function (build).  Below is an example of
what a Pkgfile file might look like.  The example shows how to package
the L<grep(1)> utility.  Some comments are inserted for explanation.
...
the Pkgfile in another package(s), since looking at examples is a
great way to learn.
...
Many settings of to the package build process can be adjusted by
...
This setting defines where L<pkgmk(8)> will store (if downloading) and
use source archives when building.

=item PKGMK_PACKAGE_DIR

This setting defines where L<pkgmk(8)> will create package files once
the build process is complete.

=item PKGMK_WORK_DIR

This setting defines a work area that L<pkgmk(8)> will use to build
the package.
...
This setting instructs L<pkgmk(8)> to attempt to fetch all source
...
URLs can be separated by spaces (the spaces in URL replace by C<%20>).

  PKGMK_SOURCE_DIR="/var/cache/pkgmk/sources"

This setting instructs L<pkgmk(8)> to store and find source archives
...
This setting instructs L<pkgmk(8)> to use
...
for the package is not necessarily present in package source
directory.  Instead the F<Pkgfile> contains an URL which points to a
...
Packages' sources are organized in so called collections.  There are
five different layer (or five categories) of packages.
...
package.  It contains various package maintaining utilities.  Also,
see L<revdep(1)>.
...
this collection depends B<only> on the C<core> pkgsrc collection.

=head4 xorg

This collection contains build scripts and files for the packages
required for X Window System applications.  Packages in this
collection depends on the C<system> collection.

=head4 desktop

This collection contains build scripts and files for the packages
typically used on a desktop: web browser, music/video player, file
manager, chat messenger, etc.  Packages in this collection depends on
...
depend on any of the collections listed above.
...
pkgsrc collections (F<usr/src/pkgsrc*>).  To obtains the collection
you need at first time, use C<git clone>.  Example:

  # cd /usr/src/
  # git clone https://github.com/zeppe-lin/pkgsrc-core

The same for other collections from the list:
...
To bring your local C<core> collection up to date use C<git pull>.  Example:
...
The same for other collections from the list:
  ...
  # cd /usr/src/pkgsrc-xorg     &&  git pull
  # cd /usr/src/pkgsrc-desktop  &&  git pull
  # cd /usr/src/pkgsrc-stuff    &&  git pull
...
=head3 Listing local pkgsrc repositories

When the local pkgsrc repositories has been cloned/updated the
directory C</usr/src/> will contain some C<pkgsrc-???> directories.

Under each of these directories you will find packages' sources: build
scripts and files for building binary package.  You
can simply browse around in the directory structure to find out which
packages' sources are available.  Let's take C<pkgsrc-core> as an
example:

  $ cd /usr/src/pkgsrc-core
  $ ls
  acl              gawk          libnftnl         pkgutils
  asciidoctor      gcc           libnsl           procps
  attr             gdbm          libpcre          psmisc
  autoconf         gettext-tiny  libpcre2         py3-setuptools
  automake         git           libpipeline      python3
  bash             glibc         libtirpc         rc
  bash-completion  glibc-32      libtool          rdate
  bc               gperf         libunistring     readline
  binutils         grep          libusb           README.md
  bison            groff         libyaml          rejmerge
  bzip2            gzip          linux-pam        revdep
  ca-certificates  hdparm        lz4              ruby
  capstone         iana-etc      lzo              samurai
  cmake            inetutils     m4               sed
  COPYING          intltool      make             shadow
  COPYRIGHT        iproute2      man-db           sqlite3
  coreutils        iptables      man-pages        start-stop-daemon
  cpio             kbd           man-pages-posix  sudo
  curl             kmod          meson            sysfsutils
  dash             less          mlocate          sysklogd
  db               libarchive    mpdecimal        sysvinit
  dcron            libbsd        nasm             tar
  diffutils        libcap        ncurses          tzdata
  ed               libcap-ng     openssh          usbids
  elfutils         libdevmapper  openssl          usbutils
  eudev            libffi        p5-xml-parser    util-linux
  expat            libgmp        patch            vim
  fakeroot         libmd         pciutils         wget
  file             libmnl        perl             which
  filesystem       libmpc        pkg-config       xz
  findutils        libmpfr       pkgman           zlib
  flex             libnfnetlink  pkgmk            zstd
...
Note, that B<pkgman> will list the packages' sources from all
C<pkgsrc> collections indicated in F</etc/pkgman.conf> as
C<pkgsrcdir>.
...
B<Note> that the recommended way is to use L<pkgman-install(8)>.  Here
we will only describe the steps to better understand the build
process.
...
the source are already downloaded this option is ignored).  When the
download is completed the package will be built.
...
corresponding configuration.  However, it does not hurt to describe
...
by having two C<groups> of processes with the same nice priority: your
usual running tasks on one side, and L<pkgman(1)> (and its child
processes) on other.  Now, if you could renice L<pkgman(1)> and its
children to a higher nice (i.e. lower priority!) value (default is 0),
compiling would inevitably take somewhat longer, but you could use
your workstation without noticing much difference to its usual
performance.
...
according to your needs.  See L<rc.conf(5)> for more information about
F<rc.conf> configuration file.
...
(this version is backwards compatible and understands C<DES> passwords
as well).

=head1 REPORTING BUGS

The source code for this site is available at:
L<https://github.com/zeppe-lin/zeppe-lin.github.io>.  Please note that
it is generated (by L<pod2html(1pm)>) from the handbook, that is
available at L<https://github.com/zeppe-lin/handbook>.  If you'd like
to fix some C<CSS> bugs and build files, refer to the source code of
the site, otherwise to the handbook source.
