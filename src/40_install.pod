...
=head1 4. INSTALL ZEPPE-LIN

=head2 4.1. Supported Hardware / Requirements
...
=head2 4.2. Install From RootFS Image
...
=head3 B<4.2.1. Disk partitions and filesystems>
...
=head3 B<4.2.2. LVM inside LUKS device>
...
=head3 B<Create root filesystem and setup swap area>

    # mkfs.ext4 /dev/zpln/root
    # mkswap /dev/zpln/swap
    # swapon /dev/zpln/swap

=head3 B<Mount prepared partitions>
...
=head3 B<Download rootfs tarball>
...
=head3 B<Configure the base system>
...
=head3 B<Prepare the package sources>
...
Now, it's highly recommended to update C<core> packages and
configuration files before the next step:

    (chrooted) # pkgman sysup --deps --depsort --group
    (chrooted) # rejmerge

=head3 B<Install bare metal packages>
...
=head3 B<Install the linux kernel>
...
=head3 B<Prepare initramfs image>
...
=head3 B<Exit from chroot and umount /mnt and reboot>
