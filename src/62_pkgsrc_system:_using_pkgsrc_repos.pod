...
=head3 B<Synchronizing your local pkgsrc repository>
...
=head3 B<Listing local pkgsrc repositories>
...
=head3 B<Listing version differences>
...
=head3 B<Building and installing packages>

B<Note>: The recommended way is to use L<pkgman-install(8)>.  Here
...
_
.SP
It is often not a good idea to build the packages
with root privileges.  Some recipes for building
programs (Makefiles, etc) may not be written
...
=head3 B<Build packages as an unprivileged user>
...
=head3 B<Renice pkgmk's child process>
