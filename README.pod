...
<a href="https://github.com/zeppe-lin/pkgsrc/blob/master/system/handbook/handbook.7.pod">handbook(7)</a>
manual page.
...
=head3 Download rootfs tarball
  ...
  # wget -c https://github.com/zeppe-lin/pkgsrc/releases/download/\
    2022-12-08/rootfs-2022-12-08-x86_64.tar.xz{,.sig}
...
L<mkinitramfs(8)> and updates the grub config.  We turn off the
...
L<pkgadd(8)>, L<pkgrm(8)>, and L<pkginfo(8)> are the basic package
...
Querying the package database is done using L<pkginfo(8)>.  This
...
handling.  To address this, a frontend utility called L<pkgman(1)>
exists.
...
the first thing to do is to update the sources:
...
operation.  By default, the I<core>, I<system>, I<xorg>, I<desktop>,
and I<stuff> collections are enabled.
...
L<sh(1)> script, which defines a number of variables (name, version,
...
Pkgfile for L<grep(1)> looks like this:
...
by editing the L<pkgmk(8)> configuration file F</etc/pkgmk.conf>.
Some of these configurable settings include:
...
falling back to the source URL specified in the Pkgfile.
...
To enforce this rule during maintenance see L<lint.mk|https://github.com/zeppe-lin/pkgsrc/blob/master/lint.mk> in pkgsrc repository, L<finddeps-linked(1)|https://github.com/zeppe-lin/pkgmaint/blob/master/finddeps-linked.1.pod>, and L<revdep(1)|https://github.com/zeppe-lin/revdep/blob/master/revdep.1.pod>.
...
structure (F</usr/src/pkgsrc/*>) is empty.  To obtains the sources at
first time, use C<git clone>.  Example:

  # cd /usr/src/
  # git clone https://github.com/zeppe-lin/pkgsrc

To bring your local pkgsrc up to date use C<git pull>.
Example:

  # cd /usr/src/pkgsrc
  # git pull

=head3 Listing local pkgsrc repository

When the local pkgsrc repository has been updated the directory
C</usr/src/pkgsrc/> will contain some package categories, C<core>,
...
Pay attention to the files C<lint.mk> and C<rootfs.mk> file in the
root directory:

  $ cd /usr/src/pkgsrc
  $ ls
  COPYING    core/     lint.mk    README.rootfs.md  stuff/   xorg/
  COPYRIGHT  desktop/  README.md  rootfs.mk         system/

F<lint.mk> will check packages' sources for some typical errors.  Type
C<make -f lint.mk help> for more information.  F<rootfs.mk> will allow
you to build Zeppe-Lin rootfs archive.  See F<README.rootfs.mk> for
more information.

You can also use L<pkgman(1)> with command B<list> and option B<--all>
to list all local packages' sources.
Example:
...
To find out if the pkgsrc repository carries packages that
are different (likely newer) compared to the versions currently
installed you can use L<pkgman-diff(1)>.  If versions differences are
found, the output could look like this:
...
The L<pkgman(1)> already uses it by default in Zeppe-Lin installation.
...
its child processes) on other.  Now, if you could renice L<pkgman(1)>
...
default, L<dhcpcd(8)> run on all interfaces.  You can specify the
interface, by modifying the C<OPTS> value in F</etc/rc.d/dhcpcd>
service file.
...
Please note that it's generated (by L<pod2html(1)>) from the handbook,
that is available at L<https://github.com/zeppe-lin/handbook>.  If
you'd like to fix some C<CSS> bugs and generation C<Makefile> refer to
the source code of the site, otherwise to the handbook source.
